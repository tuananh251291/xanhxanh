...
export default function TodayChecklist() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // kind=DARK_ROOM_CHECK ("Kiểm tra kho tối") hiện riêng ở thẻ "Công việc hàng ngày" trên Dashboard
      // (2 nhiệm vụ nhỏ, không phải 1 checkbox đơn) + trang /dark-room-check — không lặp lại ở đây.
      setItems(Array.isArray(data) ? data.filter((i: ChecklistItem) => i.kind === "SIMPLE") : []);
    } finally {
      setLoading(false);
    }
  }, []);
