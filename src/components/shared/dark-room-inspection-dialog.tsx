...
// Nhiệm vụ nhỏ "Kiểm tra kho cá nhân" (thuộc checklist "Kiểm tra kho tối") — NV kho mô chọn 1 NV cấy mô +
// 0 hoặc nhiều lỗi vi phạm tìm thấy, bấm "Hoàn tất kiểm tra". Xong 1 lượt là tính xong nhiệm vụ nhỏ 1
// trong ngày (không bắt buộc đi hết mọi NV cấy mô) — có thể mở lại nhiều lần/ngày nếu muốn kiểm tra thêm.
export default function DarkRoomInspectionDialog({
  ...
  const submit = async () => {
    if (!staffOption) { toast.error("Chưa chọn NV cấy mô"); return; }
    setSubmitting(true);
    try {
      const res = await fetch("/api/dark-room-inspection", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ checklistItemId, staffId: staffOption.value, violationTypeIds: selectedTypeIds }),
      });
      const json = await res.json();
      if (!res.ok) { toast.error(json.message ?? "Có lỗi xảy ra"); return; }
      toast.success(
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 mt-2">
          <div className="space-y-1">
            <Label className="text-sm">NV cấy mô được kiểm tra</Label>
            <Combobox
              items={staffOptions}
              value={staffOption}
