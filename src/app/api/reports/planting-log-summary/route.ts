...
// Báo cáo "Dữ liệu nhật ký cấy" — chỉ Admin/Admin cấp cao. Lọc theo khu sản xuất (User.workplaceWarehouseId
// của NV cấy mô), nhân sự, mã cây (lọc ở mức DailyRecordItem qua Lot.plantTypeId — 1 DailyRecord coi như
// 1 mã cây, chỉ cần CÓ dòng khớp là tính cả motherUsed của record đó, xem vòng lặp bên dưới), và khoảng
// thời gian theo TUẦN (Thứ 2 - Chủ nhật, weekStartsOn:1 — khớp quy ước tuần dùng chung toàn hệ thống,
// KHÔNG dùng số tuần ISO) hoặc THÁNG (lịch).
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ rangeStart, rangeEnd, rows, summary });
}
