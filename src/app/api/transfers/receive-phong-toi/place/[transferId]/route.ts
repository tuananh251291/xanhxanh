...
import { lotSelect, buildStagePreview, confirmStage } from "@/lib/receive-phong-toi";
import { z } from "zod";
...
const confirmSchema = z.object({ stage: z.enum(["THANH_PHAM", "MAU_ME"]) });
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ transferId: string }> }) {
  ...
  const matchingItems = transfer.items.filter((i) => i.lot.stage === parsed.data.stage);
