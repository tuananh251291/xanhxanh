...
// "Khoản thưởng khác" — HR tự nhập cho 1 NV cấy mô, 1 kỳ lương cụ thể, cộng thẳng vào Tổng thu nhập (xem
// src/lib/payroll-calculation.ts). ?periodMonth= lọc theo kỳ (không truyền = xem toàn bộ, mới nhất trước).
export async function GET(req: NextRequest) {
  ...
  const rows = await prisma.payrollOtherBonus.findMany({
...
export async function POST(req: NextRequest) {
  ...
  const bonus = await prisma.payrollOtherBonus.create({
