...
import { getTaskMonth, getForecastStatus, applyForecastEntry, computeMotherShortfallRows } from "@/lib/mother-forecast";
import { z } from "zod";
...
// Nộp "Dự kiến đáp ứng mẫu mẹ" LẦN ĐẦU (và DUY NHẤT) cho lộ trình 3 tháng hiện tại — mirror y hệt
// POST /api/rooting-forecast/submit (xem comment ở đó), đổi domain sang MotherForecastEntry.
export async function POST(req: NextRequest) {
  ...
  const existing = await prisma.motherForecastSubmission.findUnique({ where: { warehouseId_taskMonth: { warehouseId, taskMonth } } });
  if (existing) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.motherForecastSubmission.create({ data: { warehouseId, taskMonth, submittedById: session.user!.id } });
  });
