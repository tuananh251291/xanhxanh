...
export default async function DaChiaPage({
  params,
  searchParams,
}: {
  ...
  return (
    <ShelfListView
      ...
      search={search}
      page={page}
      extraWhere={{ OR: [{ plantTypeId: { not: null } }, { assignedStaffId: { not: null } }] }}
      section="DA_CHIA"
      backHref={`/warehouses/rooms/${room.id}`}
      backLabel="Về Phòng mẫu mẹ"
      heading={`${room.name} — Kho mẫu mẹ đã chia`}
      basePath={`/warehouses/rooms/${room.id}/da-chia`}
    />
  );
}
