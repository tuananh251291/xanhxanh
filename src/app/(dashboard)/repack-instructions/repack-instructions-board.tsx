...
import type { RepackInstructionStatus, UserRole } from "@prisma/client";
...
// Tách khỏi page.tsx để dùng lại được y hệt ở cả route gốc (/repack-instructions) lẫn tab "Chỉ định cấy
// xử lý" trong hub "Chỉ định cấy" của Kho mô (/instruction-quantity-edit) — xem plan gộp menu Kho mô,
// tránh trùng lặp logic. Role/userId/workplaceWarehouseId nhận qua prop thay vì tự đọc session, vì hub
// đã tự xác định role KHO_MO từ trước khi nhúng.
export default async function RepackInstructionsBoard({
  role, userId, workplaceWarehouseId,
}: {
  role: UserRole;
  userId: string;
  ...
  // Trước đây trang gốc redirect thẳng về /dashboard khi KHO_MO chưa gán kho sản xuất — nhúng trong hub
  // thì không thể redirect cả trang, hiện thông báo tại chỗ (giống inspection-lane) thay vì chặn cứng.
  if (role === "KHO_MO" && !workplaceWarehouseId) {
    return (
  ...
  if (role === "KHO_MO") where.sourceShelf = { warehouseId: workplaceWarehouseId };
