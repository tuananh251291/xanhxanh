...
export default function DailyRecordPage() {
  ...
  // lần lưu này) — trả lời "Không" thì kết thúc sớm chỉ định luôn (giống bấm "Kết thúc chỉ định sớm"), để
  // "Chỉ định của tôi" biết cần nhắc NV bàn giao MM dư nếu còn — xem POST /api/daily-records (Thứ 7/CN mới
  ...
  // Nạp lại danh sách chỉ định (tuần hiện tại) — dùng chung sau khi lưu dữ liệu hôm nay LẪN sau khi bấm
  // "Kết thúc chỉ định sớm" (cả 2 đều có thể khiến chỉ định đang chọn không còn ACTIVE nữa).
  const refreshInstructions = async () => {
    const instRes = await fetch("/api/instructions?status=ACTIVE");
  ...
  const answerSundayPrompt = async (willWorkSunday: boolean) => {
    ...
    try {
      ...
      toast.success("Đã kết thúc chỉ định — nhớ bàn giao MM dư nếu còn ở \"Chỉ định của tôi\"");
      await onEndedEarly();
    } finally {
  ...
  const doSubmitToday = async () => {
    ...
    try {
      ...
      if (json.ended) {
        ...
        // Thứ 7 (6) mà chỉ định chưa tự kết thúc qua lần lưu này — hỏi luôn có làm thêm Chủ nhật không, vì
        // đây là ngày DUY NHẤT có thể quyết định trước (Chủ nhật thì chỉ định đã tự kết thúc rồi).
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-text-secondary">
              Tổng MM đã sử dụng: <span className="font-semibold text-foreground">{fmt(projectedMotherUsed)}</span> / {fmt(selectedInst.inputMotherQuantity)} MM được giao
            </p>
            <p className="text-xs text-text-secondary">
              Tổng MM đã kiểm tra: <span className="font-semibold text-foreground">{fmt(cumulativeMotherChecked)}</span> / {fmt(selectedInst.inputMotherQuantity)} MM được giao
            </p>
          </CardHeader>
          <CardContent>
            {!todayRecord && (
            ...
            {todayRecord ? (
              ...
              <>
                ...
                <Button
                  ...
                  disabled={submitting || loading || motherCheckedExceeded || variantMismatch}
      ...
      <Dialog open={showSundayPrompt} onOpenChange={setShowSundayPrompt}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-text-secondary">
            ...
            bàn giao mẫu mẹ dư (nếu còn) cho Kho mô ở &quot;Chỉ định của tôi&quot;.
          </p>
      ...
      <Dialog open={showConfirmSubmit} onOpenChange={setShowConfirmSubmit}>
        <DialogContent className="max-w-sm">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <Button type="button" className="flex-1 bg-primary hover:bg-primary-hover" onClick={doSubmitToday}>
              Xác nhận lưu
            </Button>
          </div>
        </DialogContent>
