    ...
    <Card>
      <CardContent className="p-4 space-y-3">
        ...
        <p className="text-sm bg-info-light text-info-foreground rounded-md px-3 py-2">
          <strong>Giải trình / Lý do:</strong> {proposal.reason}
        </p>
...
export default function MotherForecastRequestsBoard() {
  ...
  const review = async (id: string, action: "approve" | "reject", reason?: string) => {
    setProcessingId(id);
    try {
      const res = await fetch(`/api/mother-forecast-edit-proposals/${id}`, {
        method: "PATCH",
