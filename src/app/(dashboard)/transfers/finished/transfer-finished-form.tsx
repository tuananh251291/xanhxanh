    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-primary-strong">Tạo phiếu bàn giao mới</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-primary-light">
                    ...
                    <th className="text-left px-4 py-3 text-primary-strong font-bold text-base">Số lô</th>
                    <th className="px-4 py-3 font-bold text-base">Thao tác</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.length === 0 ? (
                    <tr><td colSpan={3} className="px-4 py-8 text-center text-text-muted">Bấm &quot;Thêm hàng&quot; để bắt đầu</td></tr>
                  ) : (
                    rows.map((row) => (
                      <tr key={row.rowId} className="border-b last:border-0 even:bg-primary-light/30">
                        <td className="px-4 py-3">
                          {row.shelf ? (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              >
                                <SelectTrigger className="w-full sm:w-72 h-8"><SelectValue placeholder="Chọn giàn kệ..." /></SelectTrigger>
                                <SelectContent>
                                  {pickableShelves
                                    .filter((s) => !filledShelves.some((sc) => sc.id === s.id))
                                    .map((s) => (
                              ...
                              <Button type="button" variant="outline" size="sm" className="h-8" onClick={() => openScannerForRow(row.rowId)}>
                                <ScanLine className="w-3.5 h-3.5 mr-1.5" /> Quét QR
                              </Button>
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-3 text-text-secondary">{row.shelf ? `${row.shelf.lots.length} lô` : "—"}</td>
                        <td className="px-4 py-3 text-right">
                          <Button variant="ghost" size="sm" className="h-8" onClick={() => removeRow(row.rowId)}>
      ...
      {aggregated.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-lg font-bold text-primary-strong">Tổng hợp theo loại cây</h2>
          <Card>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-primary-light">
                      ...
                      <th className="text-right px-4 py-3 text-primary-strong font-bold text-base">T01</th>
                      <th className="text-right px-4 py-3 text-primary-strong font-bold text-base">T05</th>
                    </tr>
                  </thead>
                  <tbody>
                    {aggregated.map((row) => (
                      <tr key={row.code} className="border-b last:border-0 even:bg-primary-light/30">
                        ...
                        <td className="px-4 py-3 text-right font-medium">{row.t01.toLocaleString("vi-VN")}</td>
                        <td className="px-4 py-3 text-right font-medium">{row.t05.toLocaleString("vi-VN")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
